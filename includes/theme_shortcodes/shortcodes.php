		...
		foreach($latest as $post) {
				...
				if ($custom_class=="news") {
					...
					if ( has_post_thumbnail($post->ID) ){
						...
						if (($type=="manufacturers") && ($manufacturersurl!="")) {
							
						$output .= '<figure class="featured-thumbnail"><a href="'.$manufacturersurl.'" title="'.get_the_title($post->ID).'">';
...
function shortcode_popular_posts($atts, $content = null) {
		...
		foreach($popular as $post){
			...
			$url = $attachment_url['0'];
			$image = aq_resize($url, 300, 160, true);
				...
				$output .= '<li>';
						...
						$output .= '<div class="excerpt">';
							$output .= my_string_limit_words($excerpt,16);
						$output .= '</div>';
				$output .= '</li>';
...
function shortcode_audio($atts, $content = null) {
    ...
    if($mp3 !== false){
        ...
        _e("The URL you entered is not a .mp3 file.", "theme1843");
...
function shortcode_video($atts, $content = null) {
    ...
    if($vimeo !== false){
        ...
        _e("You entered a video URL that isn't compatible with this shortcode.", "theme1843");
