...
class MY_Vcard_Widget extends WP_Widget {
  function MY_Vcard_Widget() {
    $widget_ops = array('classname' => 'widget_theme1843_vcard', 'description' => __('Use this widget to add a vCard', 'theme1843'));
    $this->WP_Widget('widget_theme1843_vcard', __('My - vCard', 'theme1843'), $widget_ops);
    $this->alt_option_name = 'widget_theme1843_vcard';
  ...
  function widget($args, $instance) {
    $cache = wp_cache_get('widget_theme1843_vcard', 'widget');
    ...
    $title = apply_filters('widget_title', empty($instance['title']) ? __('vCard', 'theme1843') : $instance['title'], $instance, $this->id_base);
  ...
  <?php
    ...
    wp_cache_set('widget_theme1843_vcard', $cache, 'widget');
  }
  ...
  function update($new_instance, $old_instance) {
    ...
    if (isset($alloptions['widget_theme1843_vcard'])) {
      delete_option('widget_theme1843_vcard');
    }
  ...
  function flush_widget_cache() {
    wp_cache_delete('widget_theme1843_vcard', 'widget');
  }
  ...
  ?>
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title (optional):', 'theme1843'); ?></label>
      <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
    </p>
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('street_address')); ?>"><?php _e('Street Address:', 'theme1843'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('locality')); ?>"><?php _e('City/Locality:', 'theme1843'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('region')); ?>"><?php _e('State/Region:', 'theme1843'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('postal_code')); ?>"><?php _e('Zipcode/Postal Code:', 'theme1843'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('tel')); ?>"><?php _e('Telephone:', 'theme1843'); ?></label>
    ...
    <p>
      <label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php _e('Email:', 'theme1843'); ?></label>
...
function theme1843_widget_init() {
  register_widget('MY_Vcard_Widget');
}

add_action('widgets_init', 'theme1843_widget_init');