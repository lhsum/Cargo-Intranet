...
function widget($args, $instance) {
  ...
  $cat_posts = new WP_Query($args);
    ...
    <?php if ($instance['posttype'] == "manufacturers") {
		$manufacturersurl = $custom["my_manufacturers_url"][0];
    } ?>
    ...
    ?>
		<li class="cat_post_item-<?php echo $posts_counter; ?> clearfix">
			<?php if (has_post_thumbnail($post->ID)) : ?>
				<?php if ($instance["thumb"]) : ?>
				  ...
				  <?php if ( $instance['thumb_as_link'] ) : ?>
							<a href="<?php if (($instance['posttype'] == "manufacturers") && ($manufacturersurl!="")) {echo $manufacturersurl;} else {the_permalink();} ?>">
						<?php endif; ?>
            ...
            <?php if ( $instance['show_title'] ) : ?>
			  <?php echo $instance["before_post_title"]; ?><a class="post-title" href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php if ( $instance['show_title_date'] ) {?><time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_time('d'); ?> <span><?php the_time('M'); ?></span></time><?php }else{?><?php the_title(); ?><?php }?></a><?php echo $instance["after_post_title"]; ?>
			<?php endif; ?>
